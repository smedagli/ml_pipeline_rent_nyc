...
def go(input_args):
    ...
    with open(input_args.rf_config) as fp:
        rf_config = json.load(fp)
    run.config.update(rf_config)

    # Fix the random seed for the Random Forest, so we get reproducible results
    rf_config['random_state'] = input_args.random_seed

    ######################################
    # Use run.use_artifact(...).file() to get the train and validation artifact (args.trainval_artifact)
    # and save the returned path in train_local_pat
    trainval_local_path = run.use_artifact(input_args.trainval_artifact).file()
    # print(trainval_local_path)
    ...
    X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=input_args.val_size,
                                                      stratify=X[input_args.stratify_by],
                                                      random_state=input_args.random_seed)

    logger.info("Preparing sklearn pipeline")

    sk_pipe, processed_features = get_inference_pipeline(rf_config, input_args.max_tfidf_features)
    ...
    model_folder = 'random_forest_dir'
    model_path = os.path.join(model_folder, 'model_export')
    if not os.path.exists(model_folder):
        os.mkdir(model_folder)

    signature = infer_signature(X_val, y_val)
    mlflow.sklearn.save_model(sk_pipe, model_path,
                              serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE,
                              signature=signature, input_example=X_val.iloc[:2])

    # Upload the model to W&B
    artifact = wandb.Artifact(name=input_args.output_artifact,
                              type='model_export',
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    date_imputer = make_pipeline(SimpleImputer(strategy='constant', fill_value='2010-01-01'),
                                 FunctionTransformer(delta_date_feature, check_inverse=False, validate=False)
                                 )

    # Some minimal NLP for the "name" column
    reshape_to_1d = FunctionTransformer(np.reshape, kw_args={"newshape": -1})
    name_tfidf = make_pipeline(SimpleImputer(strategy="constant", fill_value=""),
                               reshape_to_1d,
    ...
    # 1) "preprocessor" applying the ColumnTransformer saved in `preprocessor`
    # 2) "random_forest" with the random forest instance saved in `random_forest`
    steps = [('preprocessor', preprocessor), ('random_forest', random_Forest)]
    sk_pipe = Pipeline(steps=steps)
...
def parse_args():
    ...
    parser.add_argument("--output_artifact", type=str, required=True, help="Name for the output serialized model")
